...
# 테스트를 진행할 때 이미 운영되고 있는 서버의 DB를 건드리면 곤란하므로 가상 DB를 만들고 진행한다.


class TestView(TestCase):
    def setUp(self):
        self.client = Client()
        self.user_001 = User.objects.create_user(username='user_001', password='somepassword')
        self.user_002 = User.objects.create_user(username='user_002', password='somepassword')
        ...
        # ManyToManyField는 objects.create() 안에 매개변수로 넣어지는 것이 아니라 add 함수로 추가한다. 여러 레코드를 연결할 수 있기 때문이다.
    ...
    # 내비개이션 바를 테스트 하기 위한 함수
    # test로 시작하지 않아서 자동으로 테스트되지 않는다. 다른 테스트 함수에 끼워넣는다.
    def navbar_test(self, soup):
        ...
        self.assertIn('Portfolio', navbar.text)

        logo_btn = navbar.find('a', text='Landing Page')
        ...
        logo_btn = navbar.find('a', text='Portfolio')
    ...
    def test_create_post(self):
        ...
        # staff가 아닌 user_001이 로그인 후 create_post에 접근
        self.client.login(username='user_001', password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff인 user_002 로그인
        self.client.login(username='user_002', password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        self.assertEqual(last_post.author.username, 'user_002')
    ...
    def test_update_post(self):
        ...
        self.client.login(
            username=self.user_001.username,
            password='somepassword'
        )
        response = self.client.get(update_post_url)
        self.assertEqual(response.status_code, 403)

        # 작성자 user_002가 접근하는 경우
        self.client.login(
    ...
    def test_comment_form(self):
        ...
        self.assertFalse(comment_area.find('form', id='writeComment'))
        ...
        comment_form = comment_area.find('form', id='writeComment')
        self.assertTrue(comment_form.find('textarea', id='id_content'))
        response = self.client.post(
            self.post_001.get_absolute_url() + 'new_comment/',
            {
                'content': 'user_001 comment',
            },
            follow=True
        )
        ...
        self.assertIn('user_001', new_comment_div.text)
        self.assertIn('user_001 comment', new_comment_div.text)

    def test_comment_update(self):
        comment_by_user_002 = Comment.objects.create(
            post=self.post_001,
            author=self.user_002,
            content='user_002 comment.'
        )
    ...
    def test_delete_comment(self):
        comment_by_user_001 = Comment.objects.create(
            post=self.post_001,
            author=self.user_001,
            content='user_001 comment'
        )
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('user_001 comment', comment_area.text)
